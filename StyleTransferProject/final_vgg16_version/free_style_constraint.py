...
t = time.time()
# Prepare images
content_size_limit = 512
style_size_limit = 512
...
print("Content size = ",np.shape(content_image))
...
# print("Mixture size = ",np.shape(mixed_image))
mixed_image = np.random.normal(size=[content_image.shape[0], content_image.shape[1], 3])
print("Mixture size = ",np.shape(mixed_image))
...
elapsed = time.time() - t
print("Total synthesis time = {:.2f} secs".format(elapsed))
